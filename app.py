...
def seccion_mercado():
    ...
    def scatter_posicionamiento(keyword_metrica, titulo_x):
        # Búsqueda fuzzy para saltar las diferencias de unidad (RMB vs USD)
    ...
    if not share_hist.empty:
        ...
        for emp in COMPANIES:
            ...
            fig.add_trace(go.Scatter(x=d_emp['Ronda'], y=d_emp['Valor'], mode='lines', name=emp,
                                      line=dict(color=COLOR_MAP[emp], width=4 if emp == MY_COMPANY else 1.5)))
        fig.update_layout(title=f'Inercia de Market Share — {pais_sel}', yaxis_title='% Share')
        mostrar(fig)
...
def seccion_operaciones():
    ...
    with bloque1:
        ...
        if not cap.empty:
            ...
            for col, (_, row) in zip(cols, cap.iterrows()):
                # Hardcode del título para evitar 'undefined'
    ...
    with bloque2:
        ...
        if matriz.sum().sum() > 0:
            # Heatmap con gaps y escala de Crema a Rojo
            fig3 = px.imshow(matriz.values, x=destinos, y=matriz.index, text_auto='.0f', aspect='auto', 
                             color_continuous_scale=[[0, BRAND_LIGHT], [1, BRAND_ACCENT]])
            fig3.update_coloraxes(showscale=False)
            fig3.update_xaxes(title_text='Destino (Mercado)')
            fig3.update_yaxes(title_text='Origen (Planta)')
            fig3.update_traces(xgap=3, ygap=3) # Separación clara entre bloques
    ...
    with bloque1:
        ...
        if ejes_validos:
            ...
            for i, (nombre, vals) in enumerate(ejes_validos.items()):
                ...
                fig2.add_annotation(x=0, y=i, text=f'{format_num(vmin, 1)}{suf}', showarrow=False, xshift=-35, font=dict(size=11, color=COLOR_REF_LINE))
                fig2.add_annotation(x=100, y=i, text=f'{format_num(vmax, 1)}{suf}', showarrow=False, xshift=35, font=dict(size=11, color=COLOR_REF_LINE))
                
            fig2.update_layout(yaxis=dict(tickmode='array', tickvals=list(range(len(ejes_validos))), ticktext=list(ejes_validos.keys())), xaxis=dict(range=[-15, 115]), height=400, title='Rango Competitivo (Mín/Med/Máx)')
...
def seccion_rrhh_sostenibilidad():
    # Inicializar selectores antes de usarlos para evitar NameError
    col_a, col_b = st.columns(2)
